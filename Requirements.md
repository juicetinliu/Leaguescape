...
## Database structure?
- [Users]
    - authId (string) - set by Firebase
    - username (string) - display name in the [game] lobby. Can be modified by the [user]
- [Games]
    - gameId (string) - autogenerated when creating a [game]. CANNOT be modified later after creation. Unique to each [game].
    - gamePassword (string) - autogenerated when creating a [game]. CAN be modified by the [admin] at any time.
    - createdTime (number) - timestamp when the [game] was created
    - startTime (number) - timestamp when the [game] progressed from `[Setup]` to `[Running]`
    - endTime (number) - timestamp when the [game] progressed from `[Running]` to `[End]`
    - adminId (string) - which [user] the [admin] of this [game] is. CANNOT be modified once created.
    - gameState (<`setup`|`running`|`end`>) - can only be modifed by the [admin] through the UI.
- [Players] (mainly for [CharacterLog]/ease of querying). Essentially logs who has access to what games.
    - playerId (string) - combination of authId and gameId?
    - gameId (string) - which [game] this [player] belongs to
    - authId (string) - which [user] this [player] is
...
- [Items] - all fields only modifiable by the [admin] unless said otherwise.
    - itemId (string) - uuid
    - gameId (string) - which [game] this item belongs to
...
- [Characters] - all fields only modifiable by the [admin] unless said otherwise.
    ...
    - gameId (string) - which [game] this character belongs to
    - name (string) - display field
    ...
    - gold (number) - current gold balance. Since purchases are 'approved' by the admin, we do NOT allow modification by other roles.
    - items (string) - csv of owned itemNumbers
    - ??? - more fields TBD.
- [CharacterActions] - message bus + activity log for later!
    - playerId (string) - for quick lookup of logs by game later.
    - characterId (string) - who did this action. Can be empty if it wasn't the admin who performed this.
    - actionType (<`enterGame`|`exitGame`|`loginCharacter`|`logoutCharacter`|`purchaseItem`|`withdrawGold`|`depositGold`|`purchaseItemStatus`|`withdrawGoldStatus`|`depositGoldtatus`|`playerUpdated`|`itemUpdated`|`characterUpdated`>) - 
    - actionDetails (string) - delims + message (or no content if not necessary). E.g. `purchaseItem` includes itemId and quantity.
    - activityTime (number) - timestamp for when the action was taken
